...
/**
 * Gets the instance of the `DT_Porch` class.
 *
 * @since  0.1
 * @access public
 * @return object|bool
...
function dt_porch() {
    ...
    if ( $is_theme_dt && version_compare( $version, $dt_porch_required_dt_theme_version, "<" ) ) {
        add_action( 'admin_notices', 'dt_porch_hook_admin_notice' );
    ...
    return DT_Porch::instance();
}
add_action( 'after_setup_theme', 'dt_porch', 20 );
...
class DT_Porch {
    ...
    public function __call( $method = '', $args = array() ) {
        _doing_it_wrong( "dt_porch::" . esc_html( $method ), 'Method does not exist.', '0.1' );
        unset( $method, $args );
        return null;
    }
}
...
register_activation_hook( __FILE__, [ 'DT_Porch', 'activation' ] );
register_deactivation_hook( __FILE__, [ 'DT_Porch', 'deactivation' ] );


if ( ! function_exists( 'dt_porch_hook_admin_notice' ) ) {
    function dt_porch_hook_admin_notice() {
        global $dt_porch_required_dt_theme_version;
        $wp_theme = wp_get_theme();
        $current_version = $wp_theme->version;
        $message = "'Disciple Tools - Porch' plugin requires 'Disciple Tools' theme to work. Please activate 'Disciple Tools' theme or make sure it is latest version.";
        if ( $wp_theme->get_template() === "disciple-tools-theme" ){
            $message .= ' ' . sprintf( esc_html( 'Current Disciple Tools version: %1$s, required version: %2$s' ), esc_html( $current_version ), esc_html( $dt_porch_required_dt_theme_version ) );
        }
        // Check if it's been dismissed...
        if ( ! get_option( 'dismissed-disciple-tools-porch', false ) ) { ?>
            <div class="notice notice-error notice-disciple-tools-porch is-dismissible" data-notice="disciple-tools-porch">
                <p><?php echo esc_html( $message );?></p>
            </div>
            <script>
                jQuery(function($) {
                    $( document ).on( 'click', '.notice-disciple-tools-porch .notice-dismiss', function () {
                        $.ajax( ajaxurl, {
                            type: 'POST',
                            data: {
                                action: 'dismissed_notice_handler',
                                type: 'disciple-tools-porch',
    ...
    if ( is_admin() && !( is_multisite() && class_exists( "DT_Multisite" ) ) || wp_doing_cron() ){
        ...
        if ( class_exists( 'Puc_v4_Factory' ) ){
            Puc_v4_Factory::buildUpdateChecker(
                'https://raw.githubusercontent.com/DiscipleTools/disciple-tools-porch/master/version-control.json',
                __FILE__,
